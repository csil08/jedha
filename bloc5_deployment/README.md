...
* [Price prediction API](https://csil08-get-around-api.hf.space). 
  - `/docs`: interactive documentation available via Swagger UI
  `POST /predict`: accepts structured JSON input data and returns price suggestions generated by a trained XGBoost model (testable via Swagger UI)
